...
\section{Second position verbal suffixes} \label{sec:clause:2pvp}

Another set of second position elements that are important to bring up are verbal suffixes. Nuuchahnulth has a series of suffix verbs that attach to the leftmost item in their direct object. (\ref{ex:havesong}--\ref{ex:havetwolongsongs}) give an example of this using the suffix verb \textit{-naˑk} `have.' Each sentence shows a longer direct object of `have': song, two songs, two long songs. The suffix verb always attaches to the first element in the object.
...
There are a wide number of other non-clausal second position suffixes. These include elements with modal- or evidential-like meanings, such as \textit{-w̓it̓as} `going to', \textit{-miḥsa} `want to', and \textit{-ckʷii} `evidently.' I treat these elements in my grammar as though they have the same second position properties as the second position verbs. This is a complex part of the grammar and this is meant only to be a sketch of it. \cite{wojdak2003} addresses these suffixes in greater detail, and \cite{woo2007b} specifically looks at the object-marking \textit{-L.(č)ił}.

\section{Verbal aspect}
...
\[ \asort{auxiliary-unary-type-raise-super} 
 ...
 args & \< \[ synsem.local.cat \[ head & \[ aux-raise & -- \\
                                         aux & + \] \\
